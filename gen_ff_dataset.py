...
class FaceFFDataset(Dataset):
    ...
    def __init__(self, csv_file, root_dir, transform=None):
        ...
        Args:
            ...
            transform (callable, optional): Optional transform to be applied
                on a sample.
        """
        self.ff_frame = pd.read_csv(csv_file)
    ...
    def __len__(self):
        return len(self.ff_frame)

    def __getitem__(self, idx):
        img_name = os.path.join(self.root_dir,self.names[idx])
        image = io.imread(img_name)
        ff = self.ff_frame.iloc[idx, 1:].as_matrix()
        ff = ff.astype('float').reshape(1, -1) 
        sample = {'image': image, 'ff': ff}
...
class Rescale(object):
    ...
    def __call__(self, sample):
        image, ff = sample['image'], sample['ff']        
        img = transform.resize(image, self.output_size)
        return {'image': img, 'ff': ff}
...
class dataFFLoader:
    def __init__(self, config):
        ...
        self.batch_table = {4:32, 8:32, 16:32, 32:16, 64:16, 128:5, 256:1, 512:1} # change this according to available gpu memory.
    ...
    def renew(self, resl):
        ...
        self.dataset = FaceFFDataset(csv_file=self.csv_file,root_dir=self.root,transform=transforms.Compose([Rescale((self.imsize,self.imsize)),ToTensor()]))       
